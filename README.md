...
uv run hektor train-doctor            # eğitim-ÖNCESİ rakip LLM/GPU yükü taraması: GO/WARN/NO-GO (salt-okuma)
uv run hektor train                   # LoRA — SADECE ÖNIZLEME (çalıştırmaz)
uv run hektor train --run             # LoRA — yerel (smoke; ağır 4B için bulut tercih et; train-doctor otomatik çalışır)
