...
_Depo: https://github.com/alimirbagirzade/hektor · Son güncelleme: 2026-09-14 (gece doğrulaması geçti · needs-approval kapısı fail-closed ve canlı sınamada ısırdı)_
...
| Alan | Durum |
|---|---|
| Kapı (`make ci`) | **CI (Linux):** ✅ main'de yeşil — `1ceb867` ve `e99a3bb` push koşuları, PR #3 (`b24c355`) ve PR #5 (`561a06c`) koşuları success. 2026-09-07 ile 2026-09-13 arası kırmızıydı (`test_sentinel_autostart_probe` ×2; bkz. 2026-09-13 kaydı). **Yerel (Windows):** ✅ ruff format + ruff check + mypy (217 dosya) + pytest **2057 passed, 4 skipped, 2 deselected** (2026-09-13, `b24c355` ağacı, `-m "not ollama"`); PR #5'in 4 statik testi yerel tam koşuda ölçülmedi, CI'da yeşil. **Yerel ✅ tek başına kapı sayılmaz.** |
