# Registers the Hektor literature scout (discovery-only) as a Windows Scheduled Task.
...
#   Test now:  schtasks /Run /TN Hektor-LiteratureScout
#
...
$taskName = "Hektor-LiteratureScout"
...
Write-Output ("Inbox: " + $(if ($env:HEKTOR_SCOUT_INBOX_DIR) { $env:HEKTOR_SCOUT_INBOX_DIR } else { Join-Path $repo "data\literature_inbox" }))
