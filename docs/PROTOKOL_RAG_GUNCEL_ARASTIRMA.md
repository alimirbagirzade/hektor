...
|---|---|---|---|
| **Tarama (triage)** | arXiv'de RAG yöntemlerini ara → adayları izleme listesine yaz | arama + heuristik | **Projeye yerleşik** (`hektor rag-scan`) — Claude/kota YOK |
...
adversarial filtreyi geçen (gerçek + offline + düşük-risk + Hektor'te eksik) **≥1 güçlü aday**
...
| `hektor rag-scan` (CLI, `app/main.py`) | Tarama ajanı giriş noktası (`--max-per-query`, `--min-score`, `--dry-run`) |
...
| `scripts/rag-research-loop.ps1` | Zamanlayıcı sarmalayıcı: `-Mode Scan` → `hektor rag-scan`; `-Mode Integrate` → `claude -p` |
...
uv run hektor rag-scan --dry-run        # yalnız listele, yazma
