...
- **SORUN (driver.py):** zorunlu Kademe-2 av kapısı (Kural 8, her eğitimden önce) TAMAMEN
  motorun kendi stdout son satırındaki `HEKTOR_HUNT_VERDICT: PASS`'ine güveniyordu. Motor
...
- **ÇÖZÜM — yapısal kanıt + dosya-sistemi teyidi (Seçenek C+A).** Yeni
  `app/orchestration/verdict_audit.py`: motor artık `HEKTOR_HUNT_EVIDENCE` JSON bloğu
...
- **SORUN 1 (ana hedef) — sür modu FİŞE TAKILDI.** `AutoDriver.drive()` `mode` alır:
  ...
  `HEKTOR_DRIVE_VERDICT` okur, `hunt_ack` YAZMAZ → zorunlu Kademe-2 av kapısı bağımsız
...
- **Canlı doğrulama (elle, kapılı):** `hektor orchestrate-drive-live --allow-live-spawn`
...
> (`orchestration_routes.py:145`) → motor Hektor MCP araçlarını GÖRÜR, veri hattını
...
Hektor MCP araçlarını **GÖRMEZ**, veri hattını **İLERLETEMEZ**.
...
`uv run hektor orchestrate-smoke` artık İKİ bölüm: **runtime** (eskisi) +
...
**Kapatılan zafiyet:** Hektor kendi `claude -p` motorunu doğuruyor ve bu motor insanla
...
1. **CLI auth'suz** — `hektor approval-approve` doğrudan `approvals.approve()` çağırır.
2. **`.env` fallback** — env'den token SİLMEK yetmez, dotenv geri okur → **boşa EZ**.
3. **MCP** — `--disallowedTools` `mcp__*`'ı kapsamaz; `hektor` MCP proxy'si 8765'e kanal açar.
...
- **Dürüstlük:** `api_token` boşken bu katman kriptografik sınır **DEĞİL**.
  **Öneri: `HEKTOR_API_TOKEN` ata.** `include_in_schema=False` koruma değildir.

---

## 🆕 2026-07-21 — MOTOR BAĞLAMA ROADMAP'İ (araştırma, kod yok)

Yerel abonelikli motorları (Claude Code / Codex / Gemini CLI) Hektor'e bağlayıp
...
MCP sunucusu (`mcp_server/hektor_mcp.py`, OpenAPI→tool proxy) · `claude -p` alt-süreç
...
(`server.py:1917`) ve stop-all temizleme (`:1971`) insanla aynı yetkide. Yani Hektor'in
...
(`hektor_mcp.py:43`) → token açarsan MCP 401, MCP çalışsın diye kapatırsan kapı açık.

**Bulgu 4 — "motor bağlama" ekranı YOK:** CLI'lar kendi OAuth'uyla girişli; Hektor
...
- **Worktree'ler kaldırılamadı** — çalışan `hektor-web` süreci `.venv` hardlink'lerini
...
GERÇEK izole worktree kur: `cd C:/Users/sevinc/Development/hektor && git worktree add
"…/hektor-<iş>" -b <branch> origin/main` → orada `uv sync --extra dev` + gate + commit + PR +
...
2. **Taze onay** — Kural 8: `uv run hektor approval-approve <id>` (ONAYSIZ EĞİTİM ASLA)
...
Arayüzler: CLI `hektor sentinel [--history]` (fail→exit 2) · web `/api/sentinel/*` ·
...
**3) 🎯 GERÇEK 1.5B LoRA EĞİTİMİ** (`hektor_lora_15b_v1`): discipline_safe_local, 400 örnek ×
...
(production terfisi İNSAN onayı bekler — Kural 8). models/adapters/hektor_lora_15b_v1.
...
**KALAN/NOT:** HektorWeb/Update scheduled-task'leri admin gerektirdi (Registry-Run autostart
...
yeni `hektor doctor` ile canlı kanıtlandı):
...
2. **Ölü scheduled-task yolları** — `start-server.ps1 -Install` yolu kuruluma sabitliyor; repo
   taşınınca HektorWeb/HektorUpdate (03:00) ölü `~/hektor`'i çağırıp sessizce no-op
...
- **#54** — update.ps1/sh deterministik main yakınsama (parklanmış dalı zorla main'e al,
  ...
  yabancı-kopya tespiti · **YENİ `hektor doctor`** (offline drift teşhisi, sapma→exit 2).
...
- **#56·#57** — stranded yerel iş cherry-pick (citation-degenerate guard + rag-chains-work
  ...
  **değerli-stranded iş KALMADI** (yeniden avlama); rag-chains-work + `hektor-chains` worktree
...
**yönetici** `start-server.ps1 -Repair`, sonra `hektor doctor` ile dal=main · +0/-0 doğrula.
...
3. **Web LoRA chat entegrasyonu** (kullanıcı web'den soru sorabilir):
   ...
   - Varsayılan: `hektor_lora_qwen15b` (1.5B). Base "(eğitimsiz, 4B)" etiketi.
...
- `hektor_lora_qwen15b` adapter'ı eşzamanlı oturumda eğitildi (1.5B, discipline_safe_local).
- **HektorWeb scheduled task** yanlış yolda (`C:\Users\sevinc\hektor`); kullanıcı onayı bekliyor.
...
`Desktop\RAG Kaynak\RLM\hektor_alexzhang_rlm_claude_integration_prompt` (1051 satır) uçtan uca
...
- **NOT:** `rlms` paketi dev `.venv`'de kurulu kaldı (#46 doğrulaması; eşzamanlı hektor-web.exe
...
- `app/web/server.py` → 3 uç (`/api/training/dataset`, `/dry-run`, `/colab-notebook`) artık
  ...
  kaynakla AYNI. `DatasetBuilder` yalnız manuel `hektor dataset` (SQLite inceleme) için kaldı.
...
**Not:** CLI `hektor dataset` ve `/api/training/run`→`launch()` zaten kanonik yolu kullanıyordu;
...
- `app/config/settings.py` → `upload_rate_limit_per_min` 20→**60** (env: `HEKTOR_UPLOAD_RATE_LIMIT_PER_MIN`).
...
(b) zaten diske inmiş ama indekslenememiş dosyalar için **"⟳ TÜMÜNÜ İNDEKSLE"** / `uv run hektor ingest`
...
> 1300'e ulaşınca: `hektor lora-split` yenile → Kaggle "Run All" tıkla (tek manuel adım).
...
- **1300'e ulaşınca:** `uv run hektor lora-split` → Kaggle "Run All"
...
- **CLI:** `hektor exam-l3 / exam-l4 / exam-l5 / understanding-score`
...
- Belgeler: README "🧠 Hektor okuduğunu *anladı* mı?" + `docs/PROTOKOL_RAG_LORA_ZINCIR.md`
...
4. **RAFT reçetesini düzelt** (seed'i yüzlerce örneğe ölçekle) → SONRA eğit (körlemesine 47h retrain YOK).
   ...
   - ✅ **#3 OFFLINE KAPI yapıldı:** `app/training/dataset_quality.py` + `hektor pretrain-gate` —
...
- **🔴 hektor_lora_v5 EĞİTİLDİ ama REJECT:** lokal CPU PEFT, 1203 adım, **46.75 saat**,
  loss 2.66→0.60. Adapter: `models/adapters/hektor_lora_v5/`. **AMA base-vs-adapter
...
- **🔧 EVAL HARNESS FIX (push'lu):** `app/training/adapter_eval.py` + `hektor lora-eval
...
- **▶️ ÖĞRENME DÖNGÜSÜ ÇALIŞIYOR** (2026-06-16 restart, 72h, `continuous-learning.sh`):
  ...
  `keep_alive=5m` (eğitim bitti). Yeni LoRA eğitimi başlatınca `.env: HEKTOR_OLLAMA_KEEP_ALIVE=0` yap.
...
- `.env`: `HEKTOR_MAX_UPLOAD_MB=100` · Fable 5 erişimi YOK (hesap/plan).

### ▶️ SABAH SIRADAKİ İŞ: Stage 2 gerçek eğitim (~30 dk, bulut-GPU)
1. `huggingface-cli upload <kullanıcı>/hektor-lora-sft data/lora_sft/lora_sft.jsonl lora_sft.jsonl --repo-type dataset`
2. HF READ token → Kaggle Secrets (ad: `HF_TOKEN`)
3. Kaggle T4×2 + Internet ON → `notebooks/hektor_lora_stage2.ipynb` → `HF_DATASET_REPO`'ya kullanıcı adı → Run All
4. İndir GGUF + Modelfile → `ollama create hektor -f Modelfile`
5. Eval: `$env:HEKTOR_LLM_MODEL='hektor'; uv run hektor evaluate evals/discipline_core.jsonl`
Detay: `docs/PROTOKOL_BULUT_EGITIM.md` · skill: `/bulut-egitim-protokolu`

### Opsiyonel (kullanıcı tetikler)
- `reindex-contextual` → P2 aktive (korpusu yeniden-embed, sonra `.env` `HEKTOR_RAG_CONTEXTUAL_EMBED=true`)
- Cross-encoder: `uv pip install sentence-transformers` + `HEKTOR_RAG_CROSS_ENCODER=true`
...
- **Uzun vadeli hedef: lokal 120B OSS model.** (ör. Llama 3.1 405B, Qwen2.5 72B+) Ollama üzerinden çalışacak. Geçiş için tek değişiklik: `.env` dosyasında `HEKTOR_LLM_MODEL` ve `HEKTOR_OLLAMA_HOST`. Kod değişikliği gerekmez.
...
- macOS LaunchAgent (`com.hektor.web.plist`)
...
```bash
uv run hektor arxiv "momentum volatility regime" --max 10
uv run hektor lora-audit && uv run hektor lora-dataset
```
Hedef: 50+ onaylı kart.

### 3. 500-iter LoRA Eğitimi
```bash
uv run hektor train --run --iters 500
```

### 4. Paper Mastery Testi
```bash
uv run hektor mastery-queue --enqueue-all && uv run hektor mastery-queue --run-all
```
...
```bash
# Sistem
uv run hektor init / status

# Makaleler
uv run hektor ingest / arxiv "sorgu" / arxiv-sync / papers

# Araştırma
uv run hektor ask "soru" / card <id> / extract-formulas / research "soru"

# Backtest
uv run hektor backtest <csv> / pine [strateji]

# Eğitim
uv run hektor dataset / chain-dataset / unified-dataset
uv run hektor mastery-to-sft
uv run hektor train / train --run
uv run hektor tool-use-train / tool-use-dataset
uv run hektor reward-analyze / auto-research

# Paper Mastery
uv run hektor mastery-run <paper_id>
uv run hektor mastery-queue [--enqueue-all|--run-next|--run-all]
uv run hektor mastery-score <paper_id>
uv run hektor mastery-report <paper_id>

# LoRA Control Plane
uv run hektor lora-status       # pipeline genel durumu
uv run hektor lora-audit        # Gate 0-8 denetle
uv run hektor lora-dataset      # dataset oluştur (--dry-run varsayılan)
uv run hektor lora-registry     # adapter kayıtları listele
