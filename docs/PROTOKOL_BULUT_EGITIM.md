...
## Ön koşul (GATE)
- ≥1000 örnek (`uv run hektor lora-readiness`) **VE** `lora-audit` geçti **VE**
  kullanıcı onayı. Eğitim yalnız açıkça başlatılır (CLAUDE.md kural 8).

## Hazırlık (lokal, tek komut)
```bash
uv run hektor lora-cloud-prep --hf-repo KULLANICI/hektor-lora-sft \
...
2. **HF private dataset:** `huggingface-cli login` (write) →
   `huggingface-cli repo create hektor-lora-sft --repo-type dataset --private` →
   `huggingface-cli upload KULLANICI/hektor-lora-sft data/lora_sft/lora_sft.jsonl lora_sft.jsonl --repo-type dataset`.
...
8. **İndir:** `hektor-Q4_K_M.gguf` + `Modelfile` + `*_loss.json`. Kaggle: Output
   sekmesi. Aynı klasöre koy (örn `models/adapters/hektor_lora_cloud/`).
9. **Ollama'ya yükle:** `ollama create hektor -f Modelfile` →
   `ollama run hektor "Sharpe oranını ve sınırlamalarını açıkla."` →
   `ollama show hektor --modelfile`. Cevap durmuyorsa `<|im_end|>` stop eksik.
10. **EVAL GATE (kural 2):**
    `$env:HEKTOR_LLM_MODEL='hektor'; uv run hektor evaluate evals/discipline_core.jsonl`.
    Hedef: score=1.0, total_flags=0 (guaranteed_profit / ignores_costs düşmemeli).
11. **Registry + PROMOTE (yalnız onayla):** `uv run hektor lora-registry` /
