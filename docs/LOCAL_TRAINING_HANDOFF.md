...
## CLI usage
```bash
uv run hektor local-training-handoff                       # son dry-run + checklist
uv run hektor local-training-handoff --approval-id apr_xxx # onayı READ-ONLY kontrol et
uv run hektor local-training-handoff --dryrun-json reports/local_training_orchestrator/..._dryrun.json
uv run hektor local-training-handoff --json
uv run hektor local-training-handoff --out reports/local_training_orchestrator
```
`ready_for_human_execution` durumunda ekrana basılır:
```text
READY FOR HUMAN EXECUTION
Recommended command:
uv run hektor train --run
...
`recommended_command = "uv run hektor train --run"` **yalnızca string** olarak döner;
...
## What it never does
`launch` · `hektor train --run` (subprocess) · `subprocess.Popen` · `os.system` ·
...
3. **İnsan** elle çalıştırır: `uv run hektor train --run` — CLI gate taze onayı **o
